--
-- Dynamic Leenium Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the leenium-shipped Plymouth via
-- leenium-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs leenium-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "leeniumunlocks"
NamePretty = "Leenium Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/leenium/themes"
  local leenium_path = os.getenv("LEENIUM_PATH") or ""
  local default_themes_dir =leenium_path .. "/themes"
  local default_preview = leenium_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "leenium-launch-floating-terminal-with-presentation "
                .. shell_escape("leenium-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "leenium-launch-floating-terminal-with-presentation "
        .. shell_escape("leenium-plymouth-reset"),
    },
  }

# Directs user to Leenium Discord
QR_CODE='
█▀▀▀▀▀█ ▄ ▄ ▀▄▄▄█ █▀▀▀▀▀█
█ ███ █ ▄▄▄▄▀▄▀▄▀ █ ███ █
█ ▀▀▀ █ ▄█  ▄█▄▄▀ █ ▀▀▀ █
...
show_log_tail() {
  if [[ -f $LEENIUM_INSTALL_LOG_FILE ]]; then
    local log_lines=$((TERM_HEIGHT - LOGO_HEIGHT - 35))
    local max_line_width=$((LOGO_WIDTH - 4))

    tail -n $log_lines "$LEENIUM_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
catch_errors() {
  ...
  gum style --foreground "${LEENIUM_COLOR_ERROR:-#ff5f56}" --padding "1 0 1 $PADDING_LEFT" "Leenium installation stopped!"
  show_log_tail

  gum style --foreground "${LEENIUM_COLOR_TEXT:-#d8f7ea}" "This command halted with exit code $exit_code:"
  show_failed_script_or_command

  gum style --foreground "${LEENIUM_COLOR_ACCENT:-#35d6a0}" "$QR_CODE"
  echo
  gum style --foreground "${LEENIUM_COLOR_MUTED:-#8cb7a9}" "Get help from the community via QR code or at https://discord.gg/k8bPTR7Yuc"
  ...
  while true; do
    ...
    case "$choice" in
    "Retry installation")
      bash ~/.local/share/leenium/install.sh
      break
      ;;
    "View full log")
      if command -v less &>/dev/null; then
        less "$LEENIUM_INSTALL_LOG_FILE"
      else
        tail "$LEENIUM_INSTALL_LOG_FILE"
      fi
      ;;

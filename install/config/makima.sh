# Remap Copilot key (Super+Shift+F23) to Super+Alt+Space (Leenium Menu) using makima
mkdir -p "$HOME/.config/makima"
cp "$LEENIUM_PATH/default/makima/AT Translated Set 2 keyboard.toml" "$HOME/.config/makima/"

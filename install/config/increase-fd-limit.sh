...
sudo tee /etc/systemd/system.conf.d/99-leenium-nofile.conf >/dev/null <<'EOF'
[Manager]
DefaultLimitNOFILESoft=65536
EOF

sudo cp /etc/systemd/system.conf.d/99-leenium-nofile.conf \
        /etc/systemd/user.conf.d/99-leenium-nofile.conf
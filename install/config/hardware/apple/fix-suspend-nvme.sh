...
if [[ $MACBOOK_MODEL =~ MacBook(8,1|9,1|10,1)|MacBookPro13,[123]|MacBookPro14,[123] ]]; then
  ...
  if [[ -f $NVME_DEVICE ]]; then
    ...
    cat <<EOF | sudo tee /etc/systemd/system/leenium-nvme-suspend-fix.service >/dev/null
[Unit]
Description=Leenium NVMe Suspend Fix for MacBook
...
EOF
    ...
    chrootable_systemctl_enable leenium-nvme-suspend-fix.service

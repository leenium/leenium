if command -v limine &>/dev/null; then
  ...
  sudo tee /etc/mkinitcpio.conf.d/leenium_hooks.conf <<EOF >/dev/null
...
EOF
  ...
  sudo cp $LEENIUM_PATH/default/limine/default.conf /etc/default/limine
  ...
  sudo cp $LEENIUM_PATH/default/limine/limine.conf /boot/limine.conf

  # Match Snapper configs if not installing from the ISO
  if [[ -z ${LEENIUM_CHROOT_INSTALL:-} ]]; then
...
if [[ -n ${LEENIUM_CHROOT_INSTALL:-} ]]; then
  echo "Leaving Limine EFI deploy hook disabled during chroot install"
  echo "Deferring limine-update until the installed system boots with EFI available"
else
  if [[ -f /usr/share/libalpm/hooks/80-limine-efi-deploy.hook.disabled ]]; then
    sudo mv /usr/share/libalpm/hooks/80-limine-efi-deploy.hook.disabled /usr/share/libalpm/hooks/80-limine-efi-deploy.hook
  fi
